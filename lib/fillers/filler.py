...
try:
    ...
    try:
        from pytorch_lightning.utilities import auto_move_data
    except ImportError:
        # PL >= 1.8 removed auto_move_data; use identity decorator
...
try:
    ...
    from pytorch_lightning.utilities.data import move_data_to_device
...
class Filler(pl.LightningModule):
    ...
    def predict_loader(self, loader, preprocess=False, postprocess=True, return_mask=True):
        ...
        for batch in loader:
            ...
            y = batch_data.pop('y')
            # Pop forecast keys (not needed for imputation inference)
            batch_data.pop('y_forecast', None)
            batch_data.pop('forecast_mask', None)
